...
- **gVisor (`runsc`) for kernel-CVE-class isolation.** Strictly stronger than sysbox-runc against host-kernel attack surface. Nested KVM isn't exposed inside dstack TDX, so the KVM platform is unavailable; the ptrace/systrap platform is the path. Provisioning is two viable shapes within the existing dstack flow — neither needs a base-image change: a prelaunch script that downloads `runsc` pinned by sha256 (script hash is in the measured launch payload), or a privileged bootstrap container in the compose whose image bakes the binary in (image digest is measured). Real work remains (install flow, perf measurement, runtime registration), but it's not blocked on coordinating with dstack. Sysbox-runc is the deployable-now layer — meaningful namespace/cap-escape protection, no kernel-CVE protection.
