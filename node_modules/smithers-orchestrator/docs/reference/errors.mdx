...
| Code | When | Details |
|---|---|---|
| `TOON_INVALID_FILE` | A TOON file cannot be parsed as a valid Smithers builder workflow definition. | -- |
| `TOON_SCHEMA_INVALID` | A TOON schema definition has an unsupported or invalid shape. | -- |
| `TOON_DUPLICATE_SCHEMA` | The same TOON schema name is declared more than once. | -- |
| `TOON_DUPLICATE_COMPONENT` | The same TOON component name is declared more than once. | -- |
| `TOON_COMPONENT_MISSING_STEPS` | A TOON component omits its required steps block. | -- |
| `TOON_DUPLICATE_STEP` | Two TOON steps or builder handles share the same id. | -- |
| `TOON_UNKNOWN_DEPENDENCY` | A TOON step depends on an id that does not exist in the workflow. | -- |
| `TOON_AGENT_CONFIG_INVALID` | A TOON agent definition is invalid or missing required properties. | -- |
| `TOON_NOT_FOUND` | A TOON schema, component, service, agent, or workflow alias cannot be resolved. | -- |
| `TOON_PLUGIN_INVALID` | A TOON plugin module does not export a valid plugin object. | -- |
| `TOON_UNKNOWN_NODE` | A TOON workflow contains an unknown or unsupported node kind. | -- |
| `TOON_STEP_MISSING_OUTPUT` | A TOON step or handle omits its declared output schema. | -- |
| `TOON_STEP_AMBIGUOUS` | A TOON step declares more than one execution mode or none at all. | -- |
| `TOON_STEP_MISSING_AGENT` | A TOON prompt step cannot resolve the agent it needs. | -- |
| `TOON_HANDLER_INVALID` | A referenced TOON handler export is not a function. | -- |
| `TOON_WORKFLOW_INVALID` | A TOON workflow definition is missing required metadata or steps. | -- |
| `TOON_NESTED_LOOP` | The builder API detects nested loop constructs that it does not support. | -- |
| `TOON_DUPLICATE_ALIAS` | Two TOON workflows declare the same alias. | -- |
| `TOON_EXECUTION_FAILED` | A TOON workflow execution result fails without a typed error payload. | -- |
